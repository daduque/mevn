...
export default {
    ...
    list: async (req, res, next) => {
        try {
            ...
            const reg = await models.Persona.find({$or:[{'nombre': new RegExp(valor, 'i')}, {'email': new RegExp(valor, 'i')}, {'num_documento': new RegExp(valor, 'i')}]},{'createdAt': 0})
            .sort({'createdAt':-1});
            res.status(200).json(reg);
        } catch(e){
            res.status(500).send({
    ...
    update: async (req, res, next) => {
        try {
            const reg = await models.Persona.findByIdAndUpdate({_id:req.query._id}, { tipo_persona: req.body.tipo_persona, nombre: req.body.nombre, tipo_documento: req.body.tipo_documento, num_documento: req.body.num_documento, direccion: req.body.direccion, telefono: req.body.telefono, email: req.body.email})
            res.status(200).json(reg);
        } catch(e){
            res.status(500).send({
                message: 'Error!!'
    ...
    enable: async (req, res, next) => {
        try {
            const reg = await models.Persona.findByIdAndUpdate({_id:req.query._id}, {estado:1});
            res.status(200).json(reg);
        } catch(e){
            res.status(500).send({
                message: 'Error!!'
    ...
    disable: async (req, res, next) => {
        try {
            const reg = await models.Persona.findByIdAndUpdate({_id:req.query._id}, {estado:0});
            res.status(200).json(reg);
